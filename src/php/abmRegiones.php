...
function buscar($buscar,$buscar2) {
 ...
 $pagina='';
 $cant=0; 
 $sql= "Select idregiones,regiones,estado from regiones where estado=? and regiones like ? "; 
 $stmt = $mysqli->prepare($sql);
 $s='ss';
 $buscar="%".$buscar."%";
 $buscar2="".$buscar2."";
...
if (!$stmt->execute()) {
 ...
 if ($valor>0) {
	  while ($valor= mysqli_fetch_assoc($result)){
		      $idregiones1=utf8_encode($valor['idregiones']);  
		  	  $regiones1=utf8_encode($valor['regiones']);
          ...
          $bacground="class='table_blanco'";
		  	  ...
		  	  $pagina.="
<tr onclick='obtener_datos_regiones_r(this)' ".$bacground." >
<td class='td_detalles' style='width:5%;background-color:#e3e2e2;color:red;text-align:center;'>".$cant."</td>
<td class='td_detalles' style='width:31%; id='td_idregiones_r'>".$idregiones1."</td>
<td class='td_detalles' style='width:31%; id='td_regiones_r'>".$regiones1."</td>
<td class='td_detalles' style='width:31%; id='td_estado_r' >".$estado."</td>
</tr>
";		  
	  }
 }
