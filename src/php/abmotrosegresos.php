...
function verificar_datos(){
		...
		if($func=='buscar'){
		 ...
		 if($func=='anular_otrosegresos'){
		    ...
		    anular_otrosegresos($nrocomprobante,$clavepermisoanulacion);	
		}
		if($func=='buscarcombo'){		
			buscar_combo();	
		}
		...
		mantenimiento($func,$sucursales_cod,$cod,$monto,$nro,$conceptos_cod,$detalles,$moneda,$caja_cod,$personas_cod,$usuarios_cod,$fecha,$hora);
		}
		}	


function mantenimiento($func,$sucursales_cod,$cod,$monto,$nro,$conceptos_cod,$detalles,$moneda,$caja_cod,$personas_cod,$usuarios_cod,$fecha,$hora){
	if($hora=="" || $monto=="" || $sucursales_cod=="" || $nro=="" || $conceptos_cod=="" || $detalles==""  || $caja_cod==""  || $personas_cod==""  ||  $usuarios_cod=="" || $fecha==""  || $hora=="" ){
    echo "camposvacio";	
    exit;
	}
...
if($func=='guardar'){
$consulta= "Select count(*) from otrosegresos where cod=?  ";
...
exit;
   ...
   $sql="insert into otrosegresos (monto, nro, personas_cod, conceptos_cod, detalles, moneda, caja_cod, fecha, hora, usuarios_cod, estado) value (upper(?),upper(?),upper(?),upper(?),upper(?),upper(?),upper(?),upper(?),upper(?),upper(?),'ACTIVO')";
   $s='ssssssssss';
   $stmt = $mysqli->prepare($sql);
   echo $mysqli -> error;
   $stmt->bind_param($s,$monto,$nro,$personas_cod,$conceptos_cod,$detalles,$moneda,$caja_cod,$fecha,$hora,$usuarios_cod); 
$consulta1="Insert into cajadiaria (origen,tipo,caja_cod, nro,egreso, descripcion, ingreso, fecha, hora, estado) 
values
('OTROSEGRESOS','OTROSEGRESOS',?,?,?,(select concat('Pago ".$detalles." ',p.primernombre,' ',p.segundonombre,' ',p.primerapellido,' ',p.segundoapellido,' ',p.apellidocasada) from personas p where p.cod='".$personas_cod."' ),'0',?,?,'ACTIVO')";	
...
if ( ! $stmt->execute()) {
 ...
 if ($valor>0)
 {
	  while ($valor= mysqli_fetch_assoc($result)) {
	  	 ...
	  	 $cant=$cant+1;
		 $accion2='ver_vetana_permiso(id_progreso,"5","","'.$nro.'")';
		  if ($cant%2==0){
...
$pagina.=" 
<tr onclick='obtener_datos_otrosegresos(this)' ".$bacground.">
...
<td class='td_detalles' style='width:30%;' id='td_detalles'>".$detalles."</td>
<td class='td_detalles' style='width:10%;text-align:center;' id='td_monto'>".number_format($monto,'0',',','.')."</td>
<td class='td_detalles' style='width:25%;' id='td_cliente'>".$cliente."</td>
...
function anular_otrosegresos($nrocomprobante,$clavepermisoanulacion){
$mysqli=conectar_al_servidor();
	if($nrocomprobante==""){
    echo "camposvacio";	
    exit;
	    ...
	    $sql="update otrosegresos set permisoanulacion='".$clavepermisoanulacion."',estado='ANULADO' where nro='".$nrocomprobante."'";
		$stmt = $mysqli->prepare($sql);
    if(!$stmt->execute()) {
     echo trigger_error('The query execution failed; MySQL said ('.$stmt->errno.') '.$stmt->error, E_USER_ERROR);
     exit;
    }
	  $sql1="update cajadiaria set permisoanulacion='".$clavepermisoanulacion."',estado='ANULADO' where nro='".$nrocomprobante."' and origen='OTROSEGRESOS'";
